...
std::wstring Kompas3D::ConfigPath() { return SystemPath(ksConfigurations); }
...
#include <KsAPI/Include/KsAPI.h>
#include <KsAPI/Include/KompasLibraryActions.h>

class Kompas3D::K3D_KS_Kompas3D : public Kompas3D::K3D_Kompas3D {
private:
	ksapi::IApplication* kompasApp = nullptr;

public:
	K3D_KS_Kompas3D(ksapi::IApplication& app, ksapi::IKompasLibraryActions& libaryActions) {
		kompasApp = &app;
		libaryActions.AddRunCommandHandler([=](uint32_t commandId, ksapi::ksRunCommandModeEnum mode) {
			Kompas3D::RunCommand(commandId);
		});
	}
	...
	void Message(const std::wstring& txt) override {
		if (kompasApp) kompasApp->ShowMessageBox(txt, L"Message", ksMessageInformation, ksButtonSetOk, false);
	}
	
	void Error(const std::wstring& txt) override {
		if (kompasApp) kompasApp->ShowMessageBox(txt, L"Error", ksMessageError, ksButtonSetOk, false);
	}
	
	std::wstring SystemPath(long type) override {
		if (kompasApp) {
			ksapi::ISystemSettingsPtr settings = kompasApp->GetSystemSettings();
			if (settings) {
				// return settings->GetSystemPath((ksSystemPathTypeEnum)type);
				return L"C:\\Users\\Ortman\\AppData\\Roaming\\ASCON\\KOMPAS-3D Viewer\\25";
			}
		}
		return L"";
	}
...
void SetKompas(std::unique_ptr<Kompas3D::K3D_Kompas3D> app) {
	Kompas3D::kompas = std::move(app);
}

APP_EXP_FUNC(bool) LoadKompasLibrary(ksapi::IApplication& app, ksapi::IKompasLibraryActions& libaryActions) {
	SetKompas(std::move(std::make_unique<Kompas3D::K3D_KS_Kompas3D>(app, libaryActions)));
	return true;
}

APP_EXP_FUNC(void) UnloadKompasLibrary() {
	SetKompas(std::make_unique<Kompas3D::K3D_Kompas3D>());
}