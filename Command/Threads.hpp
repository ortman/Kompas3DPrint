...
private:
    ...
    if (outside) {
        h = -h;
        x0 = x0 + (xPositive ? -clearance : clearance);
    } else {
        if (xyInvert) {
            y0 = y0 - h;
        } else {
    ...
    if (xyInvert) {
        ...
        drawRoundedCorner(sketch, x0, y0 + shiftY, x0 + h, y0 + shiftY + step / 2, x0, y0 + shiftY + step, abs(step / 6));
        //sketch.Line(x0, y0 + shiftY + step, x0 + h, y0 + shiftY + step / 2);
        //sketch.Line(x0 + h, y0 + shiftY + step / 2, x0, y0 + shiftY);
    }
	}
